...
class OriginEstimate:
    """
    Three-way origin probability estimate for a file or repository.

    Expresses the heuristic probability that code was:
      - fully_ai_generated_pct:  generated by an LLM in one shot
      - ai_assisted_pct:         created with AI assistance (Copilot-like)
      - human_authored_pct:      written by hand without AI tools

    IMPORTANT: These are heuristic probability estimates, NOT forensic proof.
    Never interpret as confirmed AI authorship. High AI-like patterns may
    reflect clean code, formatters, or framework conventions.

    The three percentages must sum to approximately 100.
    """

    fully_ai_generated_pct: float    # 0-100
    ai_assisted_pct: float           # 0-100
    human_authored_pct: float        # 0-100
    # Must sum to ~100
    ...
    caveats: List[str] = field(default_factory=lambda: [
        "This is a directional estimate, not forensic proof of AI authorship.",
        "High AI-like patterns may reflect clean code, formatters, or framework conventions.",
